...
from snpio.utils.misc import invert_dict
from snpio.utils.sequence_tools import get_ref_alt
...
class SNPulatorUtils:
    ...
    def validate_type(value, type, name="Value"):
        if not isinstance(value, type):
            raise TypeError(f"{name} must be of type {type}, but got {type(value)}")
    ...
    def validate_num_range(value, min=0, max=1, name="Value"):
        if value < min:
            raise ValueError(f"{name} must be in the range {min, max}, but got {value}")
        if value > max:
            raise ValueError(f"{name} must be in the range {min, max}, but got {value}")
    ...
    def check_sum_to_1(value, name="Value"):
        if isinstance(value, dict):
            ...
            raise TypeError(f"{name} must be a dict or list, but got {type(value)}")

        if value_sum != 1:
            raise ValueError(f"{name} values must sum to 1, but got {value_sum}")
...
class SNPulatoRate:
    ...
    def _calculate_base_frequencies(self):
        ...
        return self._normalize_base_frequencies(dict(sorted(base_freq.items())))
    ...
    class GTR:
        ...
        def _calculate_relative_rates(self):
            ...
            for i in range(len(alignment)):
                for j in range(i + 1, len(alignment)):
                    for pos in range(len(alignment[0])):
                        ...
                        count_matrix[base_to_index[base1], base_to_index[base2]] += 1
                        count_matrix[
            ...
            for i, base1 in enumerate(valid_bases):
                for j, base2 in enumerate(valid_bases):
                    if i < j:
                        r_ij = p_matrix[i, j] / (base_freq[base1] * base_freq[base2])
            ...
            for i in range(num_sequences):
                for j in range(i + 1, num_sequences):
                    ...
                    for pos in range(seq_length):
                        ...
                        i, j = valid_bases.index(base1), valid_bases.index(base2)
                        rate = rate_matrix[i, j]
...
class SNPulatorConfig:
    def __init__(
        ...
        Attributes:
            ...
            root_divergence_time (int or None): Divergence time of ancestral population. If provided, the mutation rates are scaled to years. If time is 1, then the mutation rates are scaled to generations. Defaults to 1.
        ...
        utils.validate_positive_number(recombination_rate, "Recombination Rate")
        ...
        if demes_graph is not None:
            if not isinstance(demes_graph, str) and not isinstance(
                ...
                raise TypeError("Demes Graph must be of type demes.Graph or str")
            elif isinstance(demes_graph, str):
                utils.validate_isfile(demes_graph, name="Demes Graph")

        if msprime_model is not None:
            utils.validate_type(msprime_model, tskit.TreeSequence, name="msprime model")
    ...
    def update_recombination_rate(self, new_rate):
        ...
        """
        SNPulatorUtils.validate_positive_number(new_rate, "New Recombination Rate")
...
class SNPulator:
    def __init__(self, genotype_data, config: SNPulatorConfig):
        ...
        self.anc_config = {k: v for k, v in self.config.items() if k in anc_kwargs}
    ...
    def initialize_demography(
        ...
        if isinstance(model, str):
            ...
            tree = model.write()
            model_obj = msprime.Demography.from_species_tree(tree, initial_size)
            ...
            raise TypeError(f"Invalid demographic model provided: {type(model)}")
    ...
    def _sim_ancestry(self, sample_sizes, populations):
        self.sample_sets = self._create_sample_sets_dict(sample_sizes, populations)
        ts = self._simulate_ancestry()
        return (
            ts,
            [f"seq{i}" for i in range(sum(sample_sizes))],
            self.replicate_list_by_dict(populations, self.sample_sets),
        )
    ...
    def replicate_list_by_dict(lst, rep_dict):
        new_lst = []
        for item in lst:
            new_lst.extend([item] * rep_dict.get(item, 1))
        return new_lst
    ...
    def _initialize_substitution_model(self):
        ...
        if "model" in self.mut_config:
            ...
            if m in [msprime.HKY, msprime.F84]:
                kappa = snprate._calculate_kappa(self.genotype_data.alignment)
                m_init = m(
                    kappa=kappa,
                    equilibrium_frequencies=list(
                        snprate._calculate_base_frequencies().values()
                    ),
                )
            elif m == msprime.GTR:
                gtr = snprate.GTR(
                    self.genotype_data.alignment,
                    snprate._calculate_base_frequencies(),
                    self.time,
                )
    ...
    def simulate_alignment(self, sample_sizes, populations):
        ...
        ) = self._sim_ancestry(sample_sizes, populations)
        self._initialize_substitution_model()
        ts = self._sim_mutations(ts)

        genotype_data.snp_data = self.get_genotypes(ts)
        genotype_data = self._set_genotypedata_attributes(genotype_data)
        return genotype_data

    def _set_genotypedata_attributes(self, genotype_data):
        genotype_data.popmap = dict(
            zip(genotype_data.samples, genotype_data.populations)
        )
        genotype_data.popmap_inverse = invert_dict(genotype_data.popmap)

        genotype_data.ref, genotype_data.alt = get_ref_alt(genotype_data.snp_data)

        genotype_data.loci_indices = list(range(len(genotype_data.snp_data[0])))
        genotype_data.sample_indices = list(range(len(genotype_data.snp_data)))
        genotype_data.q = None
        genotype_data.site_rates = None
        genotype_data.iqtree = None
        genotype_data.site_rates_iqtree_df = None
        genotype_data.tree = None
        return genotype_data

    def sim_unlinked_snps(
        self,
        sample_sizes,
        populations,
        num_sites,
    ):
        """Simulate unlinked SNP datasets.

        Args:
            num_sites (int): Number of unlinked SNP sites to simulate.
        """
        self.sample_sets = self._create_sample_sets_dict(sample_sizes, populations)

        anc_config = deepcopy(self.anc_config)
        anc_config.pop("sequence_length", None)
        anc_config.pop("demography", None)

        ts_iterator = msprime.sim_ancestry(
            samples=self.sample_sets,
            demography=self.demography,
            sequence_length=1,
            num_replicates=num_sites + 10000,
            **anc_config,
        )
        ...
        for ts in ts_iterator:
            full_genotypes = []
            mt = msprime.sim_mutations(tree_sequence=ts, **self.mut_config)
            for variant in mt.variants(
                isolated_as_missing=False, alleles=tskit.ALLELES_ACGT
            ):
            ...
            mt_matrix = np.array(full_genotypes)

            try:
                mts = (
                    np.concatenate((mts, mt_matrix), axis=0) if mts.size else mt_matrix
                )
            except ValueError:
                continue

            if mts.shape[0] == num_sites:
                break

        dfout = pd.DataFrame(mts.T)
        dfout.to_csv(
            "/Users/btm002/Documents/wtd/GeoGenIE/data/simulated_unlinked.csv",
            header=False,
            index=False,
        )

        genotype_data = self.genotype_data.copy()
        genotype_data.snp_data = mts.T.tolist()
        genotype_data = self._set_genotypedata_attributes(genotype_data)
        return genotype_data

    def simulate_alignment_replicates(
        self,
        sample_sizes,
        populations,
        num_replicates,
        ...
        self.sample_sets = dict(zip(populations, sample_sizes))
        dirpath = os.path.join(
            f"{self.genotype_data.prefix}_output",
            "gtdata",
            "alignments",
            "simulations",
        )
        self.simulated_alignment_dir = dirpath

        if verbose:
            print("\nSimulating alignments...\n\n")
            print(f"Writing simulated alignments to {dirpath}")

        Path(dirpath).mkdir(exist_ok=True, parents=True)
        filepath = os.path.join(dirpath, "simulated_alignment")
        self.ts_file_path = filepath
        self._initialize_substitution_model()
        self._validate_parallel_settings(parallel, n_jobs)

        if parallel:
            self._sim_ancestry_replicates_parallel(num_replicates, n_jobs, dirpath)
        else:
            self._sim_ancestry_replicates_sequential(num_replicates, dirpath)
    ...
    def _sim_ancestry_replicates_parallel(self, num_replicates, n_jobs, dirpath):
        # Generate random seeds
        rng = np.random.RandomState(42)
        seeds = rng.randint(1, 2**31, size=num_replicates)

        # Create a list of filenames to save the tree sequences
        filenames = [
            os.path.join(dirpath, f"replicate_{i + 1}.ts")
            for i in range(num_replicates)
        ]

        with ProcessPoolExecutor(max_workers=n_jobs) as executor:
            executor.map(self._parallel_simulation_worker, seeds, filenames)

    def _parallel_simulation_worker(self, seed, filename):
        ts = self._simulate_single_ancestry(seed)
        mutated_ts = self._simulate_single_mutation(ts)
        mutated_ts.dump(filename)

    def iterate_simulated_alignments(self):
        for filename in os.listdir(self.simulated_alignment_dir):
            if filename.endswith(".ts"):
                filepath = os.path.join(self.simulated_alignment_dir, filename)
                ts = tskit.load(filepath)
                yield ts

    def _sim_ancestry_replicates_sequential(self, num_replicates, dirpath):
        ancestry_reps = self._simulate_multiple_ancestries(num_replicates)

        for i, ts in enumerate(ancestry_reps):
            mutated_ts = self._simulate_single_mutation(ts)
            filepath = os.path.join(dirpath, f"replicate_{i + 1}.ts")
            mutated_ts.dump(filepath)
    ...
    class LDUtils:
        ...
        def plot_ld_decay(ld_matrix, bin_edges, genotype_data, show=False):
            ...
            plot_dir = os.path.join(f"{genotype_data.prefix}", "simulations", "plots")
    ...
    def set_recombination_rate(
        ...
        if strategy == "auto":
            if ld_matrix is None or bin_edges is None:
                raise ValueError(
                    "For auto strategy, ld_matrix and bin_edges must be " "provided."
                )
    ...
    def compute_ld_statistics(
        ...
        try:
            ...
            raise ModuleNotFoundError(
                "The moments package must be installed to use the LD " "functionality."
            )
    ...
    def _add_monomorphic_sites(self, tree_sequence, ancestral_states):
        ...
        existing_positions = set(site.position for site in tree_sequence.sites())
        ...
        if remaining_positions > 0:
            extra_states = np.random.choice(ancestral_states, remaining_positions)
    ...
    def _convert_to_iupac_2d(self, full_genotypes):
        ...
        for i in range(n_samples):
            ...
            for j in range(n_sites):
                ...
                first_tuple = (
                    nucleotide_tuple_list[0] if nucleotide_tuple_list else ("N", "N")
                )
    ...
    def get_genotypes(self, tree_sequence):
